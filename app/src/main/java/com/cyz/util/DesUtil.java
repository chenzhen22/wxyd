...
public class DesUtil {
    ...
    /**
     * Input Permutation. The message block is permuted by this permutation at
    ...
    /**
     * Substitution Boxes. A crucial step in the Feistel function is to perform
     * bit substitutions according to this table. A 48-bit value is split into
     * 6-bit sections, and each section is permuted into a different 6-bit value
     * according to these eight tables. (One table for each section.)
     * <p/>
     * According to Wikipedia: "The S-boxes provide the core of the security of
     ...
     * permutation to the result of the S-box substitution, in order to spread
     * the output bits across 6 different S-boxes in the next round.
     */
    ...
    /**
     * PC1 Permutation. The supplied 64-bit key is permuted according to this
     * table into a 56-bit key. (This is why DES is only a 56-bit algorithm,
     * even though you provide 64 bits of key material.)
     */
    ...
    /**
     * PC2 Permutation. The subkey generation process applies this permutation
     * to transform its running 56-bit keystuff value into the final set of 16
     * 48-bit subkeys.
     */
     ...
     * certain bit-sections of the keystuff by either one or two bits to the
     * left. This table specifies how many bits to rotate left for each of the
     * 16 steps.
     */
     ...
     * permutation table. (Note that our permutation tables, supplied by
     * Wikipedia, start counting with the left-most bit as "1".)
     */
    ...
    /**
     * Utility method to convert 8 bytes (starting at the specified offset to
     * the supplied byte array) into a single 64-bit long value. If the supplied
     * byte array does not contain 8 elements starting at offset, the missing
     * bytes are regarded as zero padding.
     */
    ...
    /**
     * Wrapper around encryptBlock() that allows arguments to be byte arrays
     * instead of longs.
     */
     ...
     * ciphertext. If the message is not a multiple of 64 bits (8 bytes), then
     ...
     * reversed, to generate a stronger key. See passwordToKey() for more
     * details.
     */
    ...
    /**
     * Convert a password string into a byte array, reversing the bits of each
     * byte to place more useful key bits into non-discarded bit-positions of
     * the 64-bit DES key input. The ever-popular 7-bit ASCII characters have
     * useful information in the least significant bit which is discarded by
     * DES, and always have zero in the most significant bit, so reversing the
     * bit order of the password bytes results in a stronger key.
     * <p/>
     * This is consistent with the "VNC Authentication" scheme used in the RFB
     * protocol:
     * <p/>
     * "The RFB specification says that VNC authentication is done by receiving
     * a 16 byte challenge, encrypting it with DES using the user specified
     * password, and sending back the resulting 16 bytes. The actual software
     * encrypts the challenge with all the bit fields in each byte of the
     * password mirrored." - http://www.vidarholen.net/contents/junk/vnc.html
     */
    ...
    /**
     * Wrapper around decryptBlock() that allows arguments to be byte arrays
     * instead of longs.
     */
     ...
     * message. If the ciphertext is not a multiple of 64 bits (8 bytes), then
     * it is padded with zeros.
     * <p/>
     * This method uses the Electronic Code Book (ECB) mode of operation -- each
     * 64-bit block is encrypted individually with the same key.
     ...
     * <p/>
     * This method uses the Cypher Block Chaining mode (CBC) mode of operation
     * -- each 64-bit block is XORed with the previous ciphertext after being
    ...
    public static String encode(String key, String value) {
        byte[] keys = DesUtil.parseBytes(key);
        String srcDataHex = DesUtil.convertStringToHex(value);
        String qrCodeEncryptData = "";
        List srcDataHexList = splitString(srcDataHex, 16);
        for (int i = 0; i < srcDataHexList.size(); i++) {
            String srcDataHexPart = (String) srcDataHexList.get(i);
            byte[] srcDataHexBytePart = DesUtil.parseBytes(srcDataHexPart);
            qrCodeEncryptData = qrCodeEncryptData.concat(DesUtil.hex(DesUtil.encryptCBC(srcDataHexBytePart, keys)));
        }
    ...
    public static String decode(String key, String value) {
        byte[] keys = DesUtil.parseBytes(key);
        List<String> listu = splitString(value, 16);
        String descu = "";
        for(int j=0; j<listu.size(); j++) {
            descu = descu.concat(DesUtil.hex(DesUtil.decryptCBC(DesUtil.parseBytes((String) listu.get(j)),keys)));
        }
        String encryptStru = "";
        for(String str : splitString(descu, 2)) {
            long longs = Long.parseLong(str, 16);
            encryptStru = encryptStru.concat((char)longs+"");
        }
        return encryptStru;
    }

    public static List<String> splitString(String inputString, int length) {
        List resList = new ArrayList();
        int remainder = (inputString.length()) % length;
        int number = (int) Math.floor((inputString.length()) / length);
        for (int index = 0; index < number; index++) {
            String childString = inputString.substring(index * length, (index + 1) * length);
            resList.add(childString);
        }
        if (remainder > 0) {
            String remainString = inputString.substring(number * length, inputString.length());
            if (remainString.length() < 16) {
                remainString = StringUtils.rightPad(remainString, 16, "0");
            }
            resList.add(remainString);
        }
        return resList;
    }
}