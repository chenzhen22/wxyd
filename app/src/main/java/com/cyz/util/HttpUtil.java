...
public class HttpUtil {
	 ...
	 public static String doGet(String httpurl) {
	    ...
	    public static Object sendAndReceiveHttps(String message, String url) throws Exception {
			...
			try {
				...
				reqConnection.setConnectTimeout(15000);
				reqConnection.setReadTimeout(60000);
				reqConnection.setDoInput(true);
				reqConnection.setDoOutput(true);
				reqConnection.setUseCaches(false);
				int responseCode;
				reqConnection.setRequestProperty("Content-Type", "text/html; charset=UTF-8");
				reqConnection.setRequestProperty("Connection", "Keep-Alive");
				responseCode = getStringLen(message, "UTF-8");
				reqConnection.addRequestProperty("Content-Length", String.valueOf(responseCode));
				...
				try {
					...
					throw new IOException("http access failed ==> " + httpURL, var19);
				}

				if (responseCode != 200) {
					throw new IOException("http access failed ==> " + httpURL + ", responseCode = "
							+ responseCode + " message=" + reqConnection.getResponseMessage());
				} else {
					...
					while (true) {
						...
						if (len <= 0) {
							String resultMsg = new String(buffer, 0, readLen, "UTF-8");
							return resultMsg;
						}
